...
(function($) {
    ...
    $.fn.datePicker = function(options) {
        return this.each(function(e) {
            var today=new Date();
            var thisYear=today.getFullYear().valueOf();
            var thisMonth=(today.getMonth()+1).valueOf();
            var thisDay=today.getDate().valueOf();
            //插件默认选项
            var that = $(this),
            ...
            function init() { //初始化函数
                ...
                if (!opts.curdate && $.trim(initVal) != '') {
                    ...
                    initY = parseInt(nowdate.getFullYear()) - parseInt(opts.beginyear) - 19,
            ...
            function createYear() {
                ...
                function yearScrollEnd(_this) {
                    ...
                    if ($('#yearScroll li').eq(initY).data('num')==thisYear){
                        if (monthScroll!=null){
                            monthScroll.destroy();
                            monthScroll=null;
                        }
            ...
            function monthScrollEnd(_this) {
                ...
                if ($('#dayScroll li').eq(initM).data('num')==thisMonth&&$('#yearScroll li').eq(initY).data('num')==thisYear) {
                    dayNum = thisDay
                }
                initM = mIndex + 1;
                if (opts.theme != 'month') {
