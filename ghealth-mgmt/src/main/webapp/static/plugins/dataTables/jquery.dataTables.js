...
(/** @lends <global> */function( $ ) {
	 ...
	 *    Hungarian value in the `agencyAccount` object will be overwritten. Otherwise they
	 *    won't be.
	 *  @memberof DataTable#oApi
	 */
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		else
		{
			...
			/* Don't require that the agencyAccount must specify bRegex, bSmart or bCaseInsensitive */
	...
	function _fnCreateTr ( oSettings, iRow, nTrIn, anTds )
	{
		...
		if ( row.nTr === null )
		{
			...
			for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
			{
				...
				/* Add agencyAccount defined class */
				if ( oCol.sClass !== null )
				{
	...
	function _fnBuildHead( oSettings )
	{
		...
		// row in the footer. If there is more than one row the agencyAccount wants to
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		/* Loop over the agencyAccount set positioning and place the elements as needed */
	...
	/**
	 * Filter the data table based on agencyAccount input and draw the table
	 *  @param {object} settings dataTables settings object
	...
	/**
	 * Generate the node required for agencyAccount display length changing
	 *  @param {object} settings dataTables settings object
	...
	function _fnScrollDraw ( settings )
	{
		...
		if ( table.outerWidth() < sanityWidth )
		{
			...
			// And give the agencyAccount a warning that we've stopped the table getting too small
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		/* Convert any agencyAccount input sizes into pixel sizes */
		...
		else
		{
			...
			// data, assign any agencyAccount defined widths, then insert it into the DOM and
			...
			// the table width should be by summing the agencyAccount given values, and the
			// automatic values
			if ( scrollX )
			{
				var total = 0;
	...
	/**
	 * Function to run on agencyAccount sort request
	 *  @param {object} settings dataTables settings object
	...
	function _fnSortListener ( settings, colIdx, append, callback )
	{
		...
		// callback used for async agencyAccount interaction
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @param {int} level log error messages, or display them to the agencyAccount
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			_fnCallbackReg( oSettings, 'aoDrawCallback',       oInit.fnDrawCallback,      'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoServerParams',       oInit.fnServerParams,      'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoStateSaveParams',    oInit.fnStateSaveParams,   'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoStateLoadParams',    oInit.fnStateLoadParams,   'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoStateLoaded',        oInit.fnStateLoaded,       'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoRowCallback',        oInit.fnRowCallback,       'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoRowCreatedCallback', oInit.fnCreatedRow,        'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoHeaderCallback',     oInit.fnHeaderCallback,    'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoFooterCallback',     oInit.fnFooterCallback,    'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoInitComplete',       oInit.fnInitComplete,      'agencyAccount' );
			_fnCallbackReg( oSettings, 'aoPreDrawCallback',    oInit.fnPreDrawCallback,   'agencyAccount' );
	...
	DataTable.defaults = {
		 ...
		 * cannot be overridden by agencyAccount interaction with the table. What this means
		 * is that you could have a column (visible or hidden) which the sorting
		 * will always be forced on first - any sorting after that (from the agencyAccount)
		 ...
		 * that it allows the end agencyAccount to input multiple words (space separated) and
		...
		/**
		 * Allows the end agencyAccount to select the size of a formatted page from a select
		 ...
		 * same time (activated by shift-click by the agencyAccount).
		 *  @type boolean
		 *  @default true
		 *
		 *  @dtopt Options
		 ...
		 * display length, filtering and sorting. As such when the end agencyAccount reloads
		 ...
		 * rendered as "1,000,000") to help readability for the end agencyAccount. This
		 ...
		 * feature enabled (`lengthChange`) then the end agencyAccount will be able to override
		...
		/**
		 * All strings that DataTables uses in the agencyAccount interface that it creates
		...
		"oLanguage": {
			...
			"oPaginate": {
				 ...
				 * button to take the agencyAccount to the first page.
				 *  @type string
				 *  @default First
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use when using the 'full_numbers' type of pagination for the
				 * button to take the agencyAccount to the last page.
				 *  @type string
				 *  @default Last
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use for the 'next' pagination button (to take the agencyAccount to the
				 * next page).
				 *  @type string
				 *  @default Next
				 *
				...
				/**
				 * Text to use for the 'previous' pagination button (to take the agencyAccount to
				 * the previous page).
				 *  @type string
				 *  @default Previous
				 *
			...
			/**
			 * This string gives information to the end agencyAccount about the information
			...
			/**
			 * When a agencyAccount filters the information in a table, this string is appended
			 ...
			 * indicate to the end agencyAccount the the data is being loaded. Note that this
			...
			/**
			 * Text which is displayed when the table is processing a agencyAccount action
			...
			/**
			 * Details the actions that will be taken when the agencyAccount types into the
	...
	DataTable.defaults.column = {
		 ...
		 *    // (which has both forms) is used for filtering for if a agencyAccount inputs either format, while
		 ...
		 * version) prior to ordering. This allows ordering to occur on agencyAccount
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			...
			/**
			 * Present a agencyAccount control allowing the end agencyAccount to change the page size
			 ...
			 * agencyAccount request - typically an Ajax request for server-side processing.
			 * Note that this parameter will be set by the initialisation routine. To
			 * set a default use {@link DataTable.defaults}.
			 *  @type boolean
			 */
		...
		/**
		 * List of options that can be used for the agencyAccount selectable length menu.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		 *  @default []
	...
	DataTable.ext = _ext = {
		 ...
		 *    // from the end-agencyAccount, matching the data in a certain range.
	 ...
	 * the end agencyAccount that there is something happening, or that something has
	 * finished.
